...
(function() {
    ...
    // --- NEW: DUST PARTICLE EFFECT ---
    function spawnDustEffect(element, direction) {
        if (!element) return;
        ...
        for(let i=0; i<100; i++) {
            ...
            p.style.width = (Math.random() * 4 + 2) + 'px'; // Random size 2-6px
            p.style.height = p.style.width;
            p.style.backgroundColor = Math.random() > 0.5 ? '#fff' : '#aaa'; // White/Grey dust
            p.style.borderRadius = '50%';
            p.style.left = (Math.random() * 100) + '%';
            p.style.top = (Math.random() * 100) + '%';
            p.style.opacity = '1';
            p.style.transition = `all ${Math.random() + 0.5}s ease-out`; // Random speed
            
            container.appendChild(p);

            // Animate flying away
            requestAnimationFrame(() => {
                const moveX = (Math.random() * 100 + 50) * (direction === 'left' ? -1 : 1);
    ...
    function applyDamage(rawDmg, sourceSide) {
        ...
        if(window.battle.enemy.hp <= 0) {
            ...
            // Trigger Fade & Dust Effect
            if (eImg) {
                eImg.classList.add('dead-anim'); // Start CSS Fade
                spawnDustEffect(eImg, 'right');  // Spawn Particles
            }
            setTimeout(handleWin, 2000); // 2s Delay
        ...
        // --- DEFEAT CHECK ---
            ...
            // Trigger Fade & Dust Effect
            if (pSprite) {
                pSprite.classList.add('dead-anim'); // Start CSS Fade
                spawnDustEffect(pSprite, 'left');   // Spawn Particles
            }
            setTimeout(handleDefeat, 2000); // 2s Delay
        }
    }
