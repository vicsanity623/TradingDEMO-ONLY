...
(function () {
    ...
    async function initGame() {
        try {
            ...
            if (typeof window.initDungeons === 'function') window.initDungeons(); // Initialize Dungeon UI if ready
    ...
    window.checkDailyLogin = function () {
        ...
        if (now - window.player.dailyLogin.lastClaimTime > oneDay || window.player.dailyLogin.lastClaimTime === 0) {
            // If more than 48 hours, reset streak to Day 1
    ...
    function doEquip() {
        ...
        if (!stackItem) { window.player.selected = -1; syncUI(); return; }
    ...
    function popDamage(dmg, id, isSpecial = false) {
        const d = document.createElement('div');
        d.className = 'pop';
        if (typeof dmg === 'string') { d.innerText = dmg; d.style.color = '#00ff00'; d.style.fontSize = '1.5rem'; }
        else { d.innerText = "-" + window.formatNumber(dmg); if (isSpecial) { d.style.color = 'cyan'; d.style.fontSize = '3rem'; d.style.zIndex = 30; } }
